...
const OrderController = {
  async create(req, res) {
    try {
      ...
      const userId = req.user._id; //  Se toma del token, no del body
      ...
      const productIdsClean = validProducts.map((product) => product._id);

      const order = new Order({
        user: userId,
        products: productIdsClean,
      });
  ...
  async getAllOrdersWithProducts(req, res) {
    try {
      const orders = await Order.find({ user: req.user._id }) // solo pedidos del usuario logueado
        .select('_id user products createdAt updatedAt')
        .populate({
          path: 'user',
          select: 'name last_name adress email',
        })
        .populate({
          path: 'products',
          select: 'name price',
